...
def init(ContextInfo):
    ...
    # 在 ContextInfo 中组合保存，供 handlebar 使用
    ...
    print(">>> 报表系统已就绪：旧数据已强制绑定 [易] 标识，新引入的 11 只其他基金保持原名输出。")
...
def handlebar(ContextInfo):
    ...
    if now_struct.tm_min % 5 == 0 and now_struct.tm_min != last_executed_minute:
        ...
        def get_trend_info(code):
            if code not in data_dict or data_dict[code].empty: return "SIDE", 0, "0.000(0.00%)"
            
            c_series = data_dict[code]['close'].iloc[:-1] 
            if len(c_series) < 60: return "SIDE", 0, "0.000(0.00%)"
            ...
            price_str = f"{curr_p:.4f}({'+' if ratio>0 else ''}{ratio:.2f}%)"
        ...
        for s in ContextInfo.my_stocks:
            t, slp, p_info = get_trend_info(s)
            cnt = update_logic(fund_persistence_dict, s, t)
            
            raw_name = ContextInfo.get_stock_name(s)
            if s[:6] in ContextInfo.raw_codes_yi:
                display_name = f"[易]{raw_name}"
            else:
                display_name = raw_name
            
            item = {'cnt': cnt, 'n': "[{}] {}".format(cnt, display_name), 's': s[:6], 'slp': slp, 'p': p_info}
        ...
        for s in ContextInfo.index_etf_list:
            ...
            item = {'cnt': cnt, 'n': "[{}] {}".format(cnt, ContextInfo.get_stock_name(s)), 's': s[:6], 'slp': slp, 'p': p_info}
        ...
        f_sum = "基金池 | 多头:{}只({:.1f}%) | 空头:{}只({:.1f}%)".format(len(f_bull), len(f_bull)/len(ContextInfo.my_stocks)*100, len(f_bear), len(f_bear)/len(ContextInfo.my_stocks)*100)
        i_sum = "指数池 | 多头:{}只({:.1f}%) | 空头:{}只({:.1f}%)".format(len(i_bull), len(i_bull)/len(ContextInfo.index_etf_list)*100, len(i_bear), len(i_bear)/len(ContextInfo.index_etf_list)*100)
        ...
        for i in range(20):
            def fmt_f(lst, idx):
                ...
                d = lst[idx]
                return f"{idx+1:<2} {d['slp']:>6.2f}%" + SEP + truncate_and_pad(d['n'], 18) + SEP + f"{d['s']} " + SEP + f"{d['p']}"
            
            def fmt_i(lst, idx):
                if idx >= len(lst): return ""
                d = lst[idx]
                return f"{d['slp']:>6.2f}%" + SEP + truncate_and_pad(d['n'], 16) + SEP + f"{d['s']} " + SEP + f"{d['p']}"
